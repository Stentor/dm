...
<?php unset($this->config['list']['layout']) ?>
  ...
  {
    return '<?php echo $this->escapeString(isset($this->config['list']['title']) ? $this->config['list']['title'] : $this->getModule()->getPlural()) ?>';
<?php unset($this->config['list']['title']) ?>
  ...
  public function getNewTitle()
  {
    return <?php
      ...
      {
        echo 'dm::getI18n()->__("Add a")." '.$this->getModule()->getName().'"';
      }
    ?>;
