...
class dmFrontLayoutHelper extends dmCoreLayoutHelper
{
  ...
  {
    if (dmConfig::get('ga_demographics'))
      $track = "ga.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'stats.g.doubleclick.net/dc.js';";
    else
      $track = "ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';";
...
(function() {
  ...
  {
    $ogtags = array(
      'og:title' => dmConfig::get('title_prefix').$this->page->get('title').dmConfig::get('title_suffix'),
      'og:url' => dmContext::getInstance()->getRequest()->getUri(),
      'og:description' => $this->page->get('description'),
      );

    $ogtags = array_merge($ogtags, $this->getService('response')->getFacebookOgTags());
    
    return $ogtags;
  }

  public function renderFacebookOgTags()
  {
    $ogHtml = '';
    if(sfConfig::get('dm_facebook_use_og_tags', false))
    {
      foreach ($this->getFacebookOgTags() as $tag => $value) 
      {
        $ogHtml.='<meta property="'.$tag.'" value="'.$value.'" />'."\n";
      }
    }
    return $ogHtml;
  }
