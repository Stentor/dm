...
class dmWidgetContentImageForm extends dmWidgetContentBaseMediaForm
{
  ...
  public function configure()
  {
    ...
    $this->widgetSchema['inline_svg'] = new sfWidgetFormInputCheckbox();
    $this->validatorSchema['inline_svg'] = new sfValidatorPass();
    $this->widgetSchema->setHelp('inline_svg', 'Embed SVG as &lt;sgv&gt; tag');
  ...
  protected function renderContent($attributes)
  {
    return $this->getHelper()->renderPartial('dmWidget', 'forms/dmWidgetContentImage', array(
      ...
      'isSvg' => (boolean) $this->getValueOrDefault('isSvg')
    ));
  }
      ...
      if ($media->isImage())
      {
        ...
        if (empty($values['width']))
        {
          if ($values['widget_width'])
          {
            ...
            {
              $values['height'] = $values['widget_width'];
            }
          }
          else
          {
