...
$this->logBlock('dm '.DIEM_VERSION.' installer', 'INFO_LARGE');

$this->logSection('dm', 'Welcome to the the dm installation wizard.');
$this->logSection('dm', 'We will now check if your server matches Symfony '.SYMFONY_VERSION.' and dm '.DIEM_VERSION.' requirements.');
...
{
  throw new Exception('We are sorry, but dm '.DIEM_VERSION.' supports only Doctrine for ORM.');
}
...
do
{
  ...
  switch($dbm)
  {
    ...
    default:
      $isDatabaseOk = false;
      $this->logBlock('dm 5.0 only supports mysql, pgsql and sqlite', 'ERROR_LARGE');
      $this->log('');
  }
  ...
  if (isset($settings['database']['dsn']))
  {
    ...
    catch (PDOException $e)
    {
      if (in_array($dbm, array('mysql', 'pgsql')) && false !== strpos($e->getMessage(), 'Unknown database'))
      {
        ...
        {
          $this->log('');
          $this->log('The database you specified does not exist, dm will now try to create it...');
...
$sendReports = false; //$this->askConfirmation(array('Send anonymous reports about plugins used to improve http://diem-project.org/plugins (Y/n)'), 'QUESTION_LARGE', true);
