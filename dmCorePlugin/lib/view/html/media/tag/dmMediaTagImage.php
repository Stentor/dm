...
class dmMediaTagImage extends dmMediaTag
{
  ...
  public function getFullMime()
  {
    return $this->resource->getFullMime();
  }
  ...
  {
    return $this->setOption('inline_svg', $svg);
  }
  ...
  public function render()
  {
    ...
    if ('image/svg+xml' === $this->getFullMime())
    {

      if($this->getOption('inline_svg'))
      {
        $tag = $this->resource->getFileContent();
        $tag = preg_replace('/<svg([.\n\w\t =":\/\-]*)>/', '<svg $1 preserveAspectRatio="xMinYMin meet">', $tag);
        //$tag='<svg'.$this->getHtmlAttributes().'></svg>';
        $tag='<div style="overflow:hidden;max-width:'.$this->getWidth().'px"><div class="svg-image">'.$tag.'</div></div>';
      }
  ...
  protected function prepareAttributesForHtml(array $attributes)
  {
    ...
    if('image/svg+xml' === $this->getFullMime() && !$this->getOption('inline_svg'))
    {
      $attributes['style'] = 'width:'.$attributes['width'].'px;height:'.$attributes['height'].'px;'.'background: url('.$attributes['src'].') no-repeat';
