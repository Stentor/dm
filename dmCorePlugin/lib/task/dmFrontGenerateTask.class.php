...
EOF;
  ...
  protected function generateLayoutTemplates()
  {
    ...
    foreach(dmDb::query('DmLayout l')->fetchRecords() as $layout)
    {
      ...
      $templateFile = dmProject::rootify('apps/front/modules/dmFront/templates/'.$template.'Success.html');
      ...
      if(!file_exists($templateFile))
      {
        ...
        {
          $filesystem->copy(
            dmOs::join(sfConfig::get('dm_front_dir'), 'modules/dmFront/templates/pageSuccess.html'),
            $templateFile
          );
    ...
    $layoutFile = dmProject::rootify('apps/front/modules/dmFront/templates/layout.html');
    ...
    if(!file_exists($layoutFile))
    {
      ...
      {
        $filesystem->copy(
          dmOs::join(sfConfig::get('dm_front_dir'), 'modules/dmFront/templates/layout.html'),
          $layoutFile
        );
